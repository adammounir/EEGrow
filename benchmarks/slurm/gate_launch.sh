...
# The pass logs are named after the job, so they follow from --jobs alone -- see the
# glob warning in section 2. Built here rather than in section 2 because the
# fallback in section 1 needs them too.
shopt -s nullglob
PASS_LOGS=()
for j in ${JOBS//,/ }; do
...
# --- 0. is the accounting database reachable at all? -------------------------
# This section exists because of a specific failure, and the failure is worth
# writing down. On 2026-08-16 this gate ran as a SLURM dependency job on marg003
# (partition `normal`) and returned RED on three checks against a smoke that was
# in fact clean -- 12/12 COMPLETED 0:0, 148 cell logs on disk. `sacct` is installed
# on the compute nodes, but slurmdbd answers only on the login node:
# AccountingStorageHost is localhost, so a compute node queries its own
# localhost:6819 and is refused.
#
#   sacct: error: _open_persist_conn: failed to open persistent connection to
#          host:localhost:6819: Connection refused
#   sacct: error: Problem talking to the database: Connection refused
#
# `2>/dev/null` discarded that, and the empty result it left behind is
# indistinguishable from "no such jobs". The emptiness then cascaded: no state, so
# no start time, so no cell logs -- one wrong assumption, three red checks, and a
# night of idle GPUs on a campaign that was ready to run.
#
# Two rules follow. Never discard sacct's stderr. And separate the two diagnoses:
# jobs unknown to a database that answered is a real failure of the smoke's
# identity, while a database we cannot reach is a limitation of where this gate
# happens to be running -- it says nothing about the smoke, and refusing on it
# makes the gate unusable from the only place SLURM can schedule it.
#
# The fallback is not a weaker guess, it is a different and arguably better
# witness: pack_run.sh prints `start=<ISO8601>` before its first sweep and
# `PACK_DONE` as its last line. Both come from the run itself.
SACCT_ERR=$(mktemp)
SACCT=$(sacct -j "$JOBS" --format=JobID%14,State%14,Elapsed,ExitCode,Start \
        -X -n -P 2>"$SACCT_ERR")
if grep -qE '_open_persist_conn|Problem talking to the database|Connection refused' "$SACCT_ERR"; then
  SACCT_OK=0
  note "sacct unreachable from $(hostname): $(sed -n 's/^sacct: error: //p' "$SACCT_ERR" | tail -1)"
  note "this is not a verdict on the smoke -- deriving state and start time from the pass logs"
elif [ -z "$SACCT" ]; then
  SACCT_OK=0
  bad "sacct answered but knows nothing of $JOBS -- the job ids are wrong"
else
  SACCT_OK=1
  [ -s "$SACCT_ERR" ] && sed 's/^/GATE    sacct: /' "$SACCT_ERR"
fi
rm -f "$SACCT_ERR"

# --- 1. every smoke job terminated cleanly -----------------------------------
if [ "$SACCT_OK" = 1 ]; then
  # -X: one row per job, not per step, so a job's own state is not confused with
  # its batch step's.
  n_job=$(wc -l <<<"$SACCT")
  n_ok=$(awk -F'|' '$2 ~ /COMPLETED/ && $4 == "0:0"' <<<"$SACCT" | wc -l)
  note "jobs: $n_ok/$n_job COMPLETED with exit 0:0 (source: sacct)"
  ...
  # A short row count means some id was silently dropped, which would otherwise
  # read as "all the jobs I know about are clean".
  [ "$n_job" = "$N_EXP" ] || bad "sacct returned $n_job row(s) for $N_EXP job id(s)"
else
  # The absence of PACK_DONE is precisely what matters: a runner killed by the
  # wall, by the OOM killer, or by one of its own fatal checks never prints it.
  # sacct's exit 0:0 only says the batch script returned zero.
  n_ok=0
  for j in ${JOBS//,/ }; do
    f="$SMOKE_LOGS/smoke_${j}.log"
    if [ -f "$f" ] && grep -q '^PACK_DONE' "$f"; then
      n_ok=$((n_ok + 1))
    elif [ -f "$f" ]; then
      echo "GATE    no PACK_DONE: job $j -- pass log present, the pass never reached its end"
    else
      echo "GATE    no PACK_DONE: job $j -- no pass log at $f"
    fi
  done
  note "jobs: $n_ok/$N_EXP reached PACK_DONE (source: pass logs)"
  [ "$n_ok" = "$N_EXP" ] || bad "$((N_EXP - n_ok)) smoke job(s) did not reach PACK_DONE"
fi
...
# PASS_LOGS is built above, before section 0, because the fallback needs it.
#
...
#
# KNOWN LIMITATION: this is a lower bound, not an interval, so judging an OLD run
# retroactively also picks up every cell log written since -- verified on 464831/464832,
# which aborted at 14:38 and were credited with the 148 cell logs of the 15:41 run. It
# is sound in the only situation the gate is used in, immediately after its own smoke,
# where no later run exists yet. Closing it would need each job's End time, which is
# exactly what the fallback has no access to.
if [ "$SACCT_OK" = 1 ]; then
  SINCE=$(awk -F'|' '{print $5}' <<<"$SACCT" | grep -v '^Unknown$' | sort | head -1)
  SINCE_SRC="sacct"
elif [ "${#PASS_LOGS[@]}" -gt 0 ]; then
  # pack_run.sh:240 -- `PACK results_dir=... cache=... start=<date -Is>`.
  SINCE=$(grep -ho 'start=[0-9T:+-]*' "${PASS_LOGS[@]}" 2>/dev/null |
          sed 's/^start=//' | sort | head -1)
  SINCE_SRC="pass logs"
else
  SINCE=""
  SINCE_SRC="nothing"
...
if [ -n "$SINCE" ]; then
  ...
  note "cell logs bounded to those written since $SINCE (source: $SINCE_SRC)"
else
  bad "cannot determine the smoke's start time from $SINCE_SRC -- refusing to date its logs by guess"
  CELL=()
fi
shopt -u nullglob
if [ "${#CELL[@]}" -eq 0 ]; then
  ...
  # awk, not `paste -sd+ | bc`: bc is absent from the compute nodes, where this gate
  # actually runs. The old form printed `bc: command not found`, left n_fatal empty,
  # and `${n_fatal:-0}` then read the missing count as zero -- so the PACK FATAL
  # check passed VACUOUSLY on every node but the login one. A check that cannot be
  # evaluated must fail, never default to clean; here it is simply made evaluable.
  # `END {print s+0}` guarantees a number even when grep matched nothing.
  n_fatal=$(grep -hc 'PACK FATAL' "${PASS_LOGS[@]}" 2>/dev/null | awk '{s += $1} END {print s+0}')
  note "OOM=$n_oom cache_faults=$n_cache tracebacks=$n_trace pack_fatal=$n_fatal"
  ...
  [ "$n_fatal" -eq 0 ] || bad "$n_fatal PACK FATAL"
