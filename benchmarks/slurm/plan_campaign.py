...
CAMPAIGN_ENV = (",RESULTS_DIR=/scratch/amounir/results_v5"
                ",LOGS=/scratch/amounir/logs/pack_v5"
                ",CLAIMS=/scratch/amounir/eegrow_claims_v5")
...
def main() -> int:
    ...
    for g_use, k in sorted(passes, reverse=True):
        ...
        line = (f"sbatch{SMOKE_SBATCH if args.smoke else ''} --gres=gpu:{g_use} "
                f"--cpus-per-task={min(args.max_cpus, max(4, g_use * k))} "
                f"--export=ALL,GRID={tsv},K={k},EEGROW_CUDA_FRACTION={cap:.3f}"
                f"{SMOKE_ENV if args.smoke else CAMPAIGN_ENV} "
                f"benchmarks/slurm/pack_run.sbatch")
